...
user_dietary_guidelines = {
    ...
    'SodiumContent': 2000/3,        # Sodium limit per meal (max 2000 mg per day)
    'CholesterolContent': 300/3,    # Cholesterol limit per meal (max 300 mg per day)
    'SaturatedFatContent': 20,      # Saturated fat limit pear meal (max 20 g per day)
...
class MealPlannerEnv(gym.Env):
    ...
    def __init__(self, df_meals: pd.DataFrame, user_dietary_guidelines, user_selected_meals=None):
        ...
        self.observation_space = gym.spaces.Box(0, self.size_meals, shape=(self.days_in_week, self.num_features), dtype=np.float64)
        # Initialize state and action mask
        self.state = np.zeros((self.days_in_week, self.num_features), dtype=np.float64)    
    ...
    def step(self, action):
        ...
        self.invalid_action_mask[action] = 0        # Selected meals get probability of 0 for future selection
    ...
    def calculate_keyword_reward(self, meal_features):
        ...
        """
        keyword_columns = self.df.columns[self.df.columns.get_loc('RecipeServings') + 1:]
        ...
        selected_keywords = pd.Series(meal_features, index=self.df.columns)[keyword_columns]
    ...
    def calculate_reward(self, terminated):
        ...
        if terminated:
            ...
            for meal_features in self.state:
                avg_protein += unscaled_value(meal_features[self.df.columns.get_loc('ProteinContent')], 'ProteinContent')
                avg_fiber += unscaled_value(meal_features[self.df.columns.get_loc('FiberContent')], 'FiberContent')
                avg_saturated_fat += unscaled_value(meal_features[self.df.columns.get_loc('SaturatedFatContent')], 'SaturatedFatContent')
            ...
            protein_content = unscaled_value(self.state[self.current_day][self.df.columns.get_loc('ProteinContent')], 'ProteinContent')
            fiber_content = unscaled_value(self.state[self.current_day][self.df.columns.get_loc('FiberContent')], 'FiberContent')
            saturated_fat_content = unscaled_value(self.state[self.current_day][self.df.columns.get_loc('SaturatedFatContent')], 'SaturatedFatContent')
            ...
            if self.user_preference_indices:
                reward += self.calculate_keyword_reward(self.state[self.current_day])
        return reward
