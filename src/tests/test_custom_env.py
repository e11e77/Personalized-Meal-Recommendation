...
def test_initialization(mock_env, mock_df):
    ...
    assert mock_env.state.shape == (mock_env.days_in_week, mock_env.num_features)
    assert mock_env.action_space.n == len(mock_df)
    assert mock_env.observation_space.shape == (mock_env.days_in_week, mock_env.num_features)
...
def test_reset(mock_env):
    ...
    assert np.array_equal(mock_env.invalid_action_mask, np.ones(mock_env.size_meals, dtype=np.int64))

def test_step(mock_env):
    for action in [0, 2]:
        mock_env.reset()
        state, reward, terminated, truncated, _ = mock_env.step(action)
        assert np.array_equal(state[mock_env.current_day - 1], mock_env.scaled_df.iloc[action].values)
...
def test_action_masks(mock_env):
    mock_env.reset()
    assert np.array_equal(mock_env.action_masks(), np.ones(mock_env.size_meals, dtype=np.int64))
    mock_env.step(0)
...
def test_reward_on_termination(mock_env):
    ...
    for day in range(mock_env.days_in_week):
        state, reward, terminated, truncated, _ = mock_env.step(day % mock_env.size_meals)
    assert terminated
    assert reward > 0  
