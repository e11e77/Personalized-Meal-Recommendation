...
hyperparameter_grid = {
    ...
    "gamma": [0.99, 0.95],                  # Discount factor for future rewards
    ...
    "eval_interval": [50],                  # Evaluation frequency during training
...
def plot_cumsum_rewards_parameters(results_array_values, param_array):
    ...
    plt.title("Cumulative Rewards Over Episodes for Different Hyperparameter Configurations")
    plt.xlabel("Episodes")
...
def plot_avg_content_parameters(results_array_values, param_array):
    ...
    - param_array (list): List of parameter configurations used for training.
    """
    labels = ['Average Protein Content', 'Average Fiber content', 'Average Saturated Fat Content']
    for i in range(len(labels)):
        plt.figure(figsize=(12, 6))
        plt.title(f"{labels[i]} Over Episodes for Different Hyperparameter Configurations")
        plt.xlabel("Episodes")
        plt.ylabel(f"{labels[i]}")
        for result, params in zip(results_array_values, param_array):
            plt.plot(result["avg_contents"][i], label=f"Config: {params['params']}")
        plt.legend()
        plt.tight_layout()
        plt.show()
...
if __name__ == '__main__':
    ...
    for linear_network in network_params:
        ...
        for params in param_combinations:
            ...
            avg_loss = np.mean([float(loss) for loss in mse_losses if loss is not None])
            total_reward = np.sum(rewards)
            cumsum_rewards = np.cumsum(rewards)
            avg_content_per_episode = calculate_average_content(agent)
            success_param = success_rates(avg_content_per_episode, num_episodes, env_py)
            ...
            print(f"Avg Reward: {avg_reward}, Avg Loss: {avg_loss}, Total Reward: {total_reward}")
        ...
        df_results.to_csv(f"hyperparameter_comparison_results_{linear_network}.csv", index=False)
